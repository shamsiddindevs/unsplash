...
const Home = () => {
  ...
  useEffect(()=>{
    if(data && data.results){
      setAllImages((prevImages)=>PAGE_NUMBER==1?data.results:[...prevImages,...data.results])
    }
  },[data])
...
//  console.log(searchParamFromAction)
  ...
  return (
    <div className="align-center">
      ...
      {allImages.length? <ImageContainer allImages={allImages}/>: (
          <div className="text-center">
            <p className="text-2xl  md:text-4xl my-10 ">No Images</p>
            <Link
              to={"/"}
        ...
        {allImages.length?<div className="my-10">
            <button onClick={addPage} className=" btn btn-sm md:btn-md   btn-secondary w-full">Read More</button>
        </div>:""}
    </div>
  );
};
